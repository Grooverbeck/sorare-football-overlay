...
export class CardIdentityLookup {
  ...
  private readonly onVisibility=():void=>{if(!document.hidden)this.schedule();};
  ...
  start():void {if(this.active)return;this.active=true;document.addEventListener('visibilitychange',this.onVisibility);this.schedule();}
  stop():void {this.active=false;document.removeEventListener('visibilitychange',this.onVisibility);this.pending.clear();if(this.timer!==undefined)window.clearTimeout(this.timer);this.timer=undefined;}
  ...
  private schedule():void {
    if(!this.active || document.hidden || this.inFlight || this.timer!==undefined || !this.pending.size) return;
    this.prune();
